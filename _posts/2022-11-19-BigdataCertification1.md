...
## 빅데이터 플랫폼 개념도
`데이터소스` > <span style='color:red'><u>`데이터수집` > `데이터 저장/처리/관리`</u></span> > <span style='color:skyblue'><u>`데이터 분석` </u></span>> `데이터 활용`  

 <span style='color:red'>관리 플랫폼</span>
 <span style='color:skyblue'>분석 플랫폼</span>
...
## <span style='color:red'><u>데이터 축소 및 차원 축소</u></span> 
...
## ETL 프로세스
데이터 원천으로 데이터를 <span style='color:red'><u>추출(Extraction)</u></span>, <span style='color:red'><u>변환(Transform)</u></span>, <span style='color:red'><u>적재(Load)</u></span>하는 작업이다.
...
> `데이터 레이크` :
> SQL유사 시스템 (HBase, NoSQL), Spark, Map Reduce, YARN, Presto로 액세스, Raw데이터, 저렴한 비용 처리  
> 접근성 매우 높음, 자유로움, 도구 및 저장용량 확장성 좋음, `스트리밍`, `빅데이터 분석 솔루션과 연동 편리`


## 하둡
~~<span style='color:red'><u>개같은 하둡</u></span>~~
### 하둡이란
...
### 수집 및 연결 프레임워크
#### 스쿱 Sqoop
> 하둡과 데이터베이스 간 <span style='color:red'><u>데이터 이동을 간편하게</u></span>  
> 하둡에서 데이터를 가져오거나 올릴 때 사용  
> 하둡에서 제공하는 데이터베이스 연결용 <span style='color:red'><u>맵 인풋 포매터</u></span>를 사용

#### 플럼 Flume
> <span style='color:red'><u>반정형데이터(로그 데이터)수집</u></span>  
> <span style='color:red'><u>비정형데이터 수집</u></span>  
> 대용량 로그 데이터를 안정성, 가용성을 바탕으로 효율적 수집  
> 다양한 소스로부터 데이터를 수집, `다양한 방식`으로 데이터를 전송 
> <span style='color:red'><u>최근 국내의 빅데이터 솔루션에서 수집 부분 채택</u></span>

#### 카프카 Kafka
> 데이터 <span style='color:red'><u>스트림 실시간 관리</u></span>를 위한 `스트리밍` 플랫폼  
> <span style='color:red'><u>발행 / 구독</u></span>모델로 구성  
> 메세지를 파일에 저장
...
> 수집된 로그 파일을 <span style='color:red'><u>HDFS</u></span>에 저장
> 지나치게 하둡에 의존

#### 스톰 Storm
> <span style='color:red'><u>실시간 스트리밍</u></span>처리 `서버이자 프레임워크`  
> 장애 대응 능력이 뛰어남  
...
### 분석 및 관리 프레임워크
#### 하이브 Hive
> 사용자가 SQL로 쿼리를 작성하면 <span style='color:red'><u>자동으로 맵리듀스 작업으로 변경</u></span>  
> 기존에 사용하던 `RDBMS`와 아주 유사한 환경의 분석 플랫폼  
> <span style='color:red'><u>자바 기반으로 널리 활용</u></span>  

#### 피그 Pig
> <span style='color:red'><u>PIG Latin</u></span>언어 사용  
> 쉬운 프로그래밍  
> 최적화와 효율 굿
...
> `HDFS`와 직접 통신  
> `하이브 쿼리 언어 HiveQL`사용  
> <span style='color:red'><u>C++</u></span>기반 실행엔진 -> 속도 빠름  
...
#### 타조 Tajo
> <span style='color:red'><u>고려대</u></span>  
> 하둡 기반의 대용량 데이터를 SQL 형태의 명령을 통해 분산 분석 작업을 진행  
> 하이브랑 비슷. `ETL`, `Low-Latency` 지원  
> 자동 최적화 지원  

#### 머하웃 Mahout
> 분산처리 가능, 확장성을 가짐 <span style='color:red'><u>머신러닝용 라이브러리</u></span>  
> 비슷한 특성을 가진 데이터를 `분류, 정의`하는 <span style='color:red'><u>작업 및 협업 필터링 수행</u></span>

#### 프레스토 Presto
> <span style='color:red'><u>Hive보다 빠른 SQL</u></span>  
> <span style='color:red'><u>자바</u></span>로 만들어짐. 메모리 처리와 데이터 구조 기술을 적절히 혼합  
> HDFS와 HBase와 같은 데이터 저장소 외에 뉴스피드 백엔드 속 <span style='color:red'><u>스토리지</u></span>와 쉽게 연결  
> <span style='color:red'><u>페이스북</u></span>은 프레스토를 여러 지역에서 운영

### 관리 프레임워크
#### 주키퍼 ZooKeeper
> 분산 시스템간의 정보 공유 및 <span style='color:red'><u>상태 체크, 동기화</u></span>처리 프레임워크  
> `코디네이션` 서비스 시스템  
> 분산 큐, 분산 로크 등 다양한 기능

#### 우지 Oozie
> 하둡의 `워크플로`관리  
> 일정한 시간이 경과하거나 <span style='color:red'><u>주기적으로 반복해서 실행</u></span>되는 잡을 관리  
> 맵리듀스 잡, 피그 잡 등 시작과 완료, 실행 중 <span style='color:red'><u>에러 등의 이벤트 콜백</u></span>

#### 휴 Hue
> Hue는 <span style='color:red'><u>코어 하둡, 스파크 하둡</u></span>에서 사용 가능한 컴포넌트  
> 하둡 클러스터와 함께 사용되는 <span style='color:red'><u>웹 기반 사용자 인터페이스</u></span>  
> 다른 하둡과 함께 그룹화되어 Hive작업 및 스파크 잡 실행

#### 에어플로우 Airflow
> 데이터 워크플로 관리 도구  
> <span style='color:red'><u>Airbnb</u></span>개발. 현재는 아파치 <span style='color:red'><u>Python</u></span>을 기반으로 코드 작성  
> HDFS와 같이 여러대의 노드가 동작해야 하는 환경에 대해서도 지원  
> <span style='color:red'><u>웹 UI 기반 강력함</u></span>  

#### 암바리 Ambari
> 손쉬움 웹 UI, REST API 사용 제공  
> <span style='color:red'><u>LINUX</u></span>기반 

#### H카탈로그 HCatalog
> <span style='color:red'><u>테이블 뷰</u></span>제공  
